# This has been modified from source here: https://huggingface.co/spaces/safetensors/convert/blob/main/convert.py
# Will now pick up a local model file instead of running up to the hugging face hub
...
def get_discard_names(model_id: str, revision: Optional[str], folder: str, token: Optional[str]) -> List[str]:
    try:
        ...
        if os.path.isdir(model_id): # Then we're local, and not on the hub
...
def convert_multi_local(model_id: str, *, revision=Optional[str], folder: str, token: Optional[str], discard_names: List[str]) -> ConversionResult:
    ...
    with open(filename, "r") as f:
        data = json.load(f)
        print(">>> pytorch_model.bin.index.json loaded")
    ...
    for filename in filenames:
        ...
        sf_filename = rename(pt_filename)
        sf_filename = os.path.join(model_id, sf_filename)
        print(f"Convert {pt_filename} to {sf_filename}")

        #convert_file(pt_filename, sf_filename, discard_names=discard_names)
        convert_file(os.path.join(model_id, pt_filename), sf_filename, discard_names=discard_names)
        local_filenames.append(sf_filename)
...
def convert(api: "HfApi", model_id: str, revision: Optional[str] = None, force: bool = False) -> Tuple["CommitInfo", List[Tuple[str, "Exception"]]]:
    ...
    if load_local is False:
        info = api.model_info(model_id, revision=revision)
        filenames = set(s.rfilename for s in info.siblings)
    else:
        filenames = {file for file in os.listdir(model_id) if os.path.isfile(os.path.join(model_id, file))}

    if load_local is True:
        if any(filename.endswith(".safetensors") for filename in filenames) and not force:
            raise AlreadyExists(f"Model {model_id} is already converted, skipping..")
        discard_names = get_discard_names(model_id, revision=revision, folder="/tmp", token=api.token)
        print("Discards", discard_names)

        if "pytorch_model.bin.index.json" in filenames:
            operations, errors = convert_multi_local(model_id, revision=revision, folder="/tmp", token=api.token, discard_names=discard_names)
        sys.exit()
